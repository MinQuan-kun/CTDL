...
public:
    ...
    ArrayList(int maxSize);         // Khởi tạo list rộng với giá trị maxsize
    ...
    vector<int> sentinel_linears(DataType data); //Tìm kiếm lính canh
    void display();
    void clear();
    int getSize();
    void displayMenu();
...
/*******************************************************************
 * @Description Tìm tất cả vị trí có giá trị data trong List bằng phương pháp lính canh
 * @parament Giá trị phần tử cần tìm
 * @return các vị trí có data
 *******************************************************************/
template <class DataType>
vector<int> ArrayList<DataType>::sentinel_linears(DataType data) {
    vector<int> indices;
    DataType sentinel = data;
    if (!isFull()) {
        _items[_iSize] = sentinel;
    }
    for (int i = 0; i <= _iSize; ++i) {
        if (_items[i] == data) {
            indices.push_back(i);
    ...
    while (true) {
        ...
        case 11:
            if (isEmpty()) {
                ...
                cin >> value;
                cin.ignore();
                vector<int> temp = sentinel_linears(value);
                if (!temp.empty()) {
                    ...
                    cout << "Khong tim thay gia tri " << value << endl;
                }
                cout << "Mang hien tai: ";
                display();
                Sleep(5000);
